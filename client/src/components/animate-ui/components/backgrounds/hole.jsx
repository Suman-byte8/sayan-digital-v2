...
function HoleBackground({
  ...
  const tick = React.useCallback(() => {
    ...
    drawParticles(ctx);
    ctx.restore();
    // eslint-disable-next-line react-hooks/immutability -- recursive rAF loop; `tick` is only invoked async, after assignment completes
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'after:content-[""] after:absolute after:z-[5] after:top-1/2 after:left-1/2 after:block after:size-full after:[background:radial-gradient(ellipse_at_50%_75%,var(--hole-halo-color)_20%,transparent_75%)] after:[transform:translate3d(-50%,-50%,0)] after:mix-blend-overlay',
        className
      )}
      style={{ '--hole-halo-color': haloColor, ...style }}
      {...props}>
      {children}
      <canvas
        ref={canvasRef}
